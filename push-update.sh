...
updates_dir=/data/horizon_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have HorizonDroid* as format"
...
adb shell "killall com.horizon.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.horizon.updater/databases/updates.db" \
