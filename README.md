...
resource (or the `horizon.updater.uri` system property) and expects as response
...
The `romtype` attribute is the string to be compared with the `ro.horizon.build_type` property. 
The `size` attribute is the size of the update expressed in bytes.  
The `url` attribute is the URL of the file to be downloaded.  
The `version` attribute is the string to be compared with the `ro.horizon.build_version` property. 
